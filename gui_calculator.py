...
class MyCalculator(QWidget):
    def __init__(self):
        ...
        # 버튼 배치
        gridLayout.addWidget(self.clearButton, 0,0,1,1) # 뒤에 1,1 은 한칸을 차지하겠다는 뜻. 앞의 두개는 row,col 좌표라고 생각하면 됨.
    ...
    def inputHistory(self, value):
        ...
        if digitValue in operator: # 사용자의 입력값이 연산자인 경우에는
            if self.input_history[-1:] in operator: 
                self.input_history += self.input_temporary + digitValue # 연산자가 입력 된 시점에서 temporary 에 기억되어 있는 숫자와 연산 기호를 합쳐서 input_history 에 넣거나
            else:
                self.input_history = self.input_temporary + digitValue # 아무것도 입력되어있지 않은 상황.
            self.input_temporary = ""
        else:
            self.input_temporary += str(digitValue)

    def replaceLastOperator(self, value):
        self.input_history = self.input_history[:-1] + str(value) # 처음부터 마지막 글자를 뺀거에 새로운 연산자를 더하는 것.
